function sortInstagramCookies(cookieStr) {
  const priorityOrder = [
    "mid",
    "ig_did",
    "datr",
  ...
  (details) => {
    ...
    for (const header of details.requestHeaders) {
      if (header.name.toLowerCase() === "cookie" && header.value) {
        if (header.value.includes("sessionid=") || header.value.includes("ds_user_id=")) {
          const formattedCookie = sortInstagramCookies(header.value);
          const dataToStore = {
            [`tab_${details.tabId}`]: formattedCookie
          };

          const uidMatch = header.value.match(/ds_user_id=(\d+)/);
