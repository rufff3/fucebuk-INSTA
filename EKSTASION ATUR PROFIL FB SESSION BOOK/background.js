chrome.commands.onCommand.addListener(async (command) => {
  if (command === "run-automation") {
    await processAutomationAllTabs();
  }
});

chrome.runtime.onMessage.addListener((req, sender, sendResponse) => {
  if (req.action === "RUN_AUTOMATION") {
    processAutomationAllTabs().then(sendResponse);
    return true;
  }
});
...
async function processAutomationAllTabs() {
  const tabs = await chrome.tabs.query({ url: "*://*.facebook.com/*" });
  if (!tabs || tabs.length === 0) {
    return { success: false, msg: "Tidak ditemukan tab Facebook yang aktif." };
  }
  ...
  const parallelPromises = tasks.map(async (task) => {
    ...
    try {
      return await chrome.scripting.executeScript({
        ...
        args: [
          task.finalBio,
          task.finalCaption,
          task.currentPostLink,
          randomPhoto,
          randomCoverPhoto,
          task.metaDistribution
        ]
      });
    } catch (err) {
      console.error(`Gagal injeksi pada tab ID ${task.tabId}:`, err);
    }
  });

  await Promise.all(parallelPromises);

  return { success: true, processedCount: tabs.length };
}

async function runFbAutomation(bioText, captionText, postLink, photoObj, coverPhotoObj, meta) {
  function renderHUD(statusText, type = "process") {
    let panel = document.getElementById("fb-automation-hud-card");
    if (!panel) {
      panel = document.createElement("div");
      panel.id = "fb-automation-hud-card";
      panel.style.cssText = `
    ...
    const shortText = (str) => {
      if (!str) return "<span style='color: #64748b;'>Tidak ada</span>";
    ...
    panel.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; border-bottom: 1px solid #21262d; padding-bottom: 6px;">
        <span style="font-weight: 700; color: #ffffff;">Distribusi Tab (${meta?.tabIndex || 1}/${meta?.totalTabs || 1})</span>
        <div style="display: flex; align-items: center; gap: 6px;">
          <span style="background: ${badgeBg}; color: #ffffff; padding: 2px 6px; border-radius: 4px; font-size: 10px; font-weight: 600;">${badgeText}</span>
          <span id="fb-hud-close" style="cursor: pointer; color: #8b949e; font-size: 13px; font-weight: bold; padding: 0 2px;">✕</span>
        </div>
      </div>
      <div style="font-size: 11px; color: #8b949e; margin-bottom: 8px; display: flex; flex-direction: column; gap: 3px;">
        <div><strong>Link Bio:</strong> <span style="color: #c9d1d9;">${shortText(meta?.bioLink)}</span></div>
        <div><strong>Link Profil:</strong> <span style="color: #c9d1d9;">${shortText(meta?.profileLink)}</span></div>
        <div><strong>Link Post:</strong> <span style="color: #c9d1d9;">${shortText(meta?.postLink)}</span></div>
      </div>
      <div style="background: #161b22; border-radius: 6px; padding: 8px; font-size: 11px; color: #f0f6fc; border-left: 3px solid ${borderCol};">
        ${statusText}
      </div>
    `;

    const closeBtn = panel.querySelector("#fb-hud-close");
    if (closeBtn) {
  ...
  async function uploadAndSetPhotoAction(t, file, caption) {
    ...
    if (!photoId) return { success: false, msg: "Gagal ekstrak ID foto profil." };
    ...
    if (txCommit.includes('"errors":') || txCommit.includes("errorSummary")) {
      return { success: false, msg: "Mutasi foto profil ditolak." };
    }
    return { success: true };
  }
  ...
  async function uploadAndSetCoverPhotoAction(t, file) {
    ...
    if (!photoId) return { success: false, msg: "Gagal ekstrak ID foto sampul." };
  ...
  renderHUD("Mengambil token & sesi akun...", "process");
  const tokens = getTokens();
  if (!tokens.f || !tokens.c) {
    renderHUD("Gagal: Token fb_dtsg atau ID pengguna tidak ditemukan.", "error");
    return;
  }

  let log = [];
  if (bioText && bioText.trim()) {
    renderHUD("Memperbarui bio akun...", "process");
    let okBio = await updateBioAction(tokens, bioText.trim());
    log.push(okBio ? "Bio: Berhasil" : "Bio: Gagal");
  }

  if (coverPhotoObj && coverPhotoObj.data) {
    renderHUD("Mengunggah foto sampul...", "process");
    ...
    log.push(resCover.success ? "Foto Sampul: Berhasil" : "Foto Sampul: Gagal");
  }

  if (photoObj && photoObj.data) {
    renderHUD("Mengunggah foto profil...", "process");
    ...
    log.push(resPhoto.success ? "Foto Profil: Berhasil" : "Foto Profil: Gagal");
  }

  if (postLink && postLink.trim()) {
    renderHUD("Membuat postingan baru...", "process");
    let okPost = await createPostAction(tokens, postLink.trim());
    log.push(okPost ? "Post: Berhasil" : "Post: Gagal");
  }

  const isFailed = log.some(r => r.includes("Gagal"));
  if (isFailed) {
    renderHUD(log.join(" | "), "error");
  } else {
    renderHUD(log.length > 0 ? log.join(" | ") : "Eksekusi selesai tanpa data baru.", "success");
  }
}